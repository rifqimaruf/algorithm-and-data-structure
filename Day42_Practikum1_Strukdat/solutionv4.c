...
// Fungsi untuk mengkonversi string simbol ke angka
int symbolStringToNum(char* symbolStr)
{
    int result = 0;
    int len = strlen(symbolStr);
    
    for(int i = 0; i < len; i++)
    {
        int digit = symbolToNum(symbolStr[i]);
        if(digit == -1) return -1; // Simbol tidak valid
...
void solveCursedMan(int n, int maxWeight, char** encryptedWeights)
{
    ...
    for (int i = 0; i < n; i++)
    {
        results[i].weights = (int*)malloc(sizeof(int) * (maxWeight + 1)); // Alokasi untuk semua berat yang mungkin
        results[i].count = 0;
    }

    int expected = maxWeight;  // Mulai dari nilai maksimum (M)
    ...
    for (int day = 0; day < n; day++)
    {
        if (results[day].count == 0) {
            printf("\n");  // Cetak baris kosong jika tidak ada ikan yang dijual
        } else {
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        encryptedWeights[i] = (char*)malloc(6); // Maksimal 5 karakter + null terminator
        scanf("%s", encryptedWeights[i]);
    }

    solveCursedMan(n, maxWeight, encryptedWeights);
