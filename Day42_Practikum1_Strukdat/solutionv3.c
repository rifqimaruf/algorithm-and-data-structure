...
void solveCursedMan(int n, char* encryptedWeights)
{
    ...
    for (int i = 0; i < n; i++)
    {
        weights[i] = symbolToNum(encryptedWeights[i]);
    }
    ...
    int expected = 9;  // Mulai dari 9 (terbesar)
    ...
    for (int day = 0; day < n; day++)
    {
        if (results[day].count == 0) {
            printf("\n");  // Cetak kosong jika tidak ada ikan yang terjual
        } else {
...
int main()
{
    ...
    char* encryptedWeights = (char*)malloc(sizeof(char) * (n + 1));
    
    // Baca simbol ikan yang dipisahkan oleh spasi
    for (int i = 0; i < n; i++)
    {
        scanf(" %c", &encryptedWeights[i]);
    }
    encryptedWeights[n] = '\0';
